function prob_out = apply_boundary_conditions_Wsq(prob_in, bcs_funcs_in, epsilon_in)
  % prob_out = apply_boundary_conditions_Wsq(prob_in, bcs_funcs_in, epsilon_in)
  %
  ...
  % Grab the indices from the manifold segment
  ...
  prob = coco_add_glue(prob, 'glue_pars_Wsq', ...
                       uidx1(maps1.p_idx(1:data_EP.pdim)), ...
                       uidxW(mapsW.p_idx));
  ...
  prob = coco_add_func(prob, 'bcs_Wsq_initial', bcs_Wsq_initial{:}, [], ...
  ...
  % @bcs_initial.
  prob = coco_add_func(prob, 'bcs_Wsq_final', bcs_Wsq_final{:}, data_EP, 'zero', 'uidx', ...
                       [uidxW(mapsW.x1_idx); ...
  ...
  % segment.
  prob = coco_add_func(prob, 'monitor_q_theta_old', @monitor_q_perturb, data_EP, ...
                       'active', {'A_perturb', 'theta_perturb'}, 'uidx', ...
                       [uidx2(maps2.x0_idx); uidxW(mapsW.x0_idx)]);

  % Add parameters to monitor the coordinates of \gamma_{\theta}
  prob = coco_add_pars(prob, 'pars_gamma_theta', uidx2(maps2.x0_idx), ...
                       {'G_theta', 'Q_theta', 'I_theta'}, 'active');

  % Add parameters to monitor the coordinates of W^{s}(q)
  prob = coco_add_pars(prob, 'pars_Wsq_theta', uidxW(mapsW.x0_idx), ...
                       {'G_Wsq', 'Q_Wsq', 'I_Wsq'}, 'active');

...
fprintf(' Previous solution label : %d\n', label_old);
fprintf(' Continuation parameters : %s\n', 'theta, T_Wsq, A_perturb, theta_perturb');
fprintf(' =====================================================================\n');
...
%------------------%
% Set continuation parameters and parameter range
pcont  = {'theta', 'T_Wsq', 'A_perturb', 'theta_perturb'};
...
% Read and print out important values
bd1 = coco_bd_read(data_run1);
bd2 = coco_bd_read(data_run2);
bdq = coco_bd_read(data_runq);

% Get labels
labs1 = coco_bd_labs(bd1, 'SP');
labs2 = coco_bd_labs(bd2, 'SP');
labsq = coco_bd_labs(bdq, 'SP');

% Intersection points
theta_old     = [coco_bd_val(bd1, labs1, 'theta'), coco_bd_val(bd2, labs2, 'theta')]';
A_perturb     = [coco_bd_val(bd1, labs1, 'A_perturb'), coco_bd_val(bd2, labs2, 'A_perturb')]';
theta_perturb = [coco_bd_val(bd1, labs1, 'theta_perturb'), coco_bd_val(bd2, labs2, 'theta_perturb')]';
I_theta       = [coco_bd_val(bd1, labs1, 'I_theta'), coco_bd_val(bd2, labs2, 'I_theta')]';

% Print
for idx = 1 : length(theta_old)
  fprintf(' =====================================================================\n');
  fprintf(' Intersection Point\n');
  fprintf(' ---------------------------------------------------------------------\n');
  fprintf(' theta_perturb : %.6f\n', theta_perturb(idx));
  fprintf(' A_perturb     : %.6f\n', A_perturb(idx));
  fprintf(' theta_old     : %.6f\n', theta_old(idx));
  fprintf(' I_theta       : %.6f\n', I_theta(idx));
  fprintf(' =====================================================================\n\n');
end
...
save_fig11_data(run_names, '../data_files/fig11_data.mat');
